...
// CREATE - Yaratish
router.post('/siz_uchun', async (req, res) => {
  try {
    const { food_ca_id } = req.body;
    const query = 'INSERT INTO siz_uchun (food_ca_id) VALUES ($1) RETURNING *';
...
// READ - O'qish
router.get('/siz_uchun/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const query = 'SELECT * FROM siz_uchun WHERE id = $1';
    const values = [id];
...
// READ - O'qish
router.get('/siz_uchun/', async (req, res) => {
    try {
      const query = 'SELECT * FROM category';
      const result = await db.query(query);
      const query1 = 'SELECT * FROM siz_uchun';
...
// UPDATE - O'zgartirish
router.put('/siz_uchun/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const { food_ca_id } = req.body;
...
// DELETE - O'chirish
router.delete('/siz_uchun/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const query = 'DELETE FROM siz_uchun WHERE id = $1';
    const values = [id];
