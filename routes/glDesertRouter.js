...
// CREATE - Yaratish
router.post('/gl_product', async (req, res) => {
  try {
    const { food_ca_id } = req.body;
    const query = 'INSERT INTO gl_product (food_ca_id) VALUES ($1) RETURNING *';
...
// READ - O'qish
router.get('/gl_product/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const query = 'SELECT * FROM gl_product WHERE id = $1';
    const values = [id];
...
// READ - O'qish
router.get('/gl_product/', async (req, res) => {
    try {
...
// UPDATE - O'zgartirish
router.put('/gl_product/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const { food_ca_id } = req.body;
    const query = 'UPDATE gl_product SET food_ca_id = $1, time_update = current_timestamp WHERE id = $2 RETURNING *';
...
// DELETE - O'chirish
router.delete('/gl_product/:id', async (req, res) => {
  try {
    const id = req.params.id;
    const query = 'DELETE FROM gl_product WHERE id = $1';
    const values = [id];
